...
async function fixture (model, data) {
  ...
  const { where, defaults } = _extractData(data)
...
function _extractData (data) {
  if (data.where && data.defaults && data.where instanceof Object && data.defaults instanceof Object) {
    return { where: data.where, defaults: data.defaults }
  } else {
    return { where: data, defaults: {} }
  }
}
