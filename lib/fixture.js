...
async function fixture (model, data, options = {}) {
  try {
    if (data instanceof Array) {
      return Promise.all(data.map((el, i) => fixture(model, el, options)))
    }
    ...
    await _executePrerequisiteAssociations(whereAssociations, baseWhere, options)

    let obj = await model.findOne({ where: _removeVirtualAttrs(baseWhere, model), ...options })
    if (obj) {
      // Run sets
      await _executePrerequisiteAssociations(setsAssociations, baseSets, options)
      obj = await obj.update(Object.assign({}, baseSets), options)
      await _promiseAllCompat([
        () => _executeDependentAssociations(whereAssociations, obj, options),
        () => _executeDependentAssociations(setsAssociations, obj, options)
      ])
      return obj
    } else {
      // Create and run defaults
      await _executePrerequisiteAssociations(defaultsAssociations, baseDefaults, options)
      obj = await model.create(Object.assign({}, baseWhere, baseDefaults), options)
      await _promiseAllCompat([
        () => _executeDependentAssociations(whereAssociations, obj, options),
        () => _executeDependentAssociations(defaultsAssociations, obj, options)
      ])
      return obj
    }
  } catch (e) {
...
function _executePrerequisiteAssociations (associations, data, options) {
  ...
  const postCreatePromises = _mapObj(belongsToAssociations, (a) => async () => {
    const record = await _executeAssociation(a.association.target, a.value, options)
...
function _executeDependentAssociations (associations, obj, options) {
  ...
  const postCreatePromises = _mapObj(hasManyAssociations, (a) => async () => {
    ...
    const record = await _executeAssociation(a.association.target, write, options)
    return record
  })
  return _objPromiseAll(postCreatePromises)
}
...
async function _executeAssociation (model, data, options) {
  if (data instanceof Array) {
    // return Promise.all(data.map(el => _executeAssociation(model, el)))
    const resolved = []
    for (let i = 0; i < data.length; ++i) {
      resolved.push(await _executeAssociation(model, data[i], options))
    }
    return resolved
    ...
    return fixture(model, data, options)
  }
}
